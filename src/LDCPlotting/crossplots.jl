...
function create_crossplot(N::Int, Re::Int, limiters::Vector{<:AbstractLimiter};
    quantity=:velocity_magnitude, figsize=(400 * length(limiters), 400 * length(limiters)), contours=true)
    ...
    for i in 1:n_limiters
        for j in i:n_limiters
            ...
            if i == j
                ...
                if contours
                    contour!(ax, x, y, quantities[limiters[i]],
                        levels=10, color=:white, linewidth=1)
                end
